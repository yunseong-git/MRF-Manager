...
export class authController {
    ...
    async login(req, res, next) {
        try {
            const { militaryId, password, isAdmin } = req.body;
            if (isAdmin == true){} //
            const user = await this.userModel.login(militaryId, password);
            if (user) {
    ...
    async registerUser(req, res, next) {
        try {
            const { militaryId, name, birthDate, password } = req.body;
            const user = await this.userModel.register(militaryId, name, birthDate, password);
            if (user) {
